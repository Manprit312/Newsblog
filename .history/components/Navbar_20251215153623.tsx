...
import { Search, Menu, X, ChevronDown, Lock, Facebook, Twitter, Instagram, Linkedin, Phone, Moon } from 'lucide-react';
...
export default function Navbar() {
  ...
  return (
    <>
      {/* Top Bar - Dark Purple */}
      <div className="bg-[#4a148c] text-white py-2 text-sm">
        <div className="container mx-auto px-4 max-w-7xl">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-4">
              <Link href="/" className="hover:text-yellow-300 transition-colors">Home</Link>
              <Link href="/about" className="hover:text-yellow-300 transition-colors">About Us</Link>
              <Link href="/advertise" className="hover:text-yellow-300 transition-colors">Advertise</Link>
          ...
          <div className="flex items-center gap-4">
              ...
              <Link href="/admin/login" className="flex items-center gap-1 hover:text-yellow-300 transition-colors">
      ...
      <nav className="bg-white border-b border-gray-200 sticky top-0 z-50 shadow-sm">
        <div className="container mx-auto px-4 max-w-7xl">
          ...
          <div className="flex items-center justify-between py-4 border-b border-gray-200">
            ...
            <Link href="/" className="flex items-center gap-3 group">
              <div className="relative">
                <div className="w-12 h-12 bg-yello-600 rounded-full flex items-center justify-center">
                  <span className="text-white font-bold text-xl">N</span>
                </div>
              </div>
              <div className="flex flex-col">
                <span className="text-2xl md:text-3xl font-normal text-gray-900 leading-tight">News</span>
                <span className="text-xl md:text-2xl font-bold text-gray-900 leading-tight">Line</span>
              </div>
            </Link>
              ...
              <form onSubmit={handleSearch} className="flex-1">
                ...
                <input
                  ...
                  onChange={(e) => setSearchQuery(e.target.value)}
                    className="w-full px-4 py-2 pl-10 border border-gray-300 rounded-md text-gray-800 focus:outline-none focus:border-yello-600 focus:ring-1 focus:ring-yello-600 transition-all text-sm"
                />
                  <Search className="absolute left-3 top-1/2 transform -translate-y-1/2 text-gray-400 w-4 h-4" />
              </div>
              </form>
              <button className="bg-yello-600 text-white px-6 py-2 rounded-md font-semibold hover:bg-yello-700 transition-colors text-sm">
                Subscribe
              </button>
            </div>
            ...
            <div className="flex items-center gap-4">
              <Phone className="w-5 h-5 text-gray-600 hidden md:block" />
              <button className="p-2 hover:bg-gray-100 rounded transition-colors" aria-label="Toggle theme">
          ...
          <div className="hidden lg:flex items-center justify-center gap-6 py-3">
            <Link
              href="/"
              className={`px-2 py-2 font-bold text-sm uppercase transition-colors border-b-2 ${
                isActive('/')
                  ? 'text-gray-900 border-yello-600'
                  : 'text-gray-700 border-transparent hover:text-gray-900 hover:border-yello-600'
              }`}
            >
              WORLD
            </Link>
            ...
            {categories.map((category) => (
              ...
              >
                <Link
                  href={category.href}
                  className={`px-2 py-2 font-bold text-sm uppercase transition-colors border-b-2 flex items-center gap-1 ${
                ...
                {category.subcategories && openDropdown === category.name && (
                  <div className="absolute top-full left-0 mt-2 bg-white shadow-lg rounded-md py-2 min-w-[200px] z-50 border border-gray-200">
                    {category.subcategories.map((sub) => (
                      <Link
                        key={sub.href}
                        href={sub.href}
                        className="block px-4 py-2.5 text-gray-700 hover:bg-gray-50 hover:text-gray-900 transition-colors text-sm"
                      >
                        {sub.name}
                      </Link>
                    ))}
            ...
            <div className="container mx-auto px-4 py-4 space-y-1">
              ...
              <form onSubmit={handleSearch} className="mb-4">
                <div className="relative">
                  <input
                    ...
                    value={searchQuery}
                    onChange={(e) => setSearchQuery(e.target.value)}
                    className="w-full px-4 py-2.5 pl-10 border border-gray-300 rounded-md text-gray-800 focus:outline-none focus:border-secondary-blue focus:ring-1 focus:ring-secondary-blue"
                  />
                  <Search className="absolute left-3 top-1/2 transform -translate-y-1/2 text-gray-400 w-4 h-4" />
                </div>
              </form>
              ...
              <Link
                ...
                className={`block px-4 py-3 font-semibold text-sm transition-colors border-l-4 ${
                  isActive('/')
                    ? 'text-secondary-blue border-secondary-blue bg-gray-50'
                    : 'text-gray-700 border-transparent hover:bg-gray-50 hover:text-secondary-blue'
                }`}
              >
                Home
              </Link>
                    ...
                    <div className="pl-6 space-y-1">
                      {category.subcategories.map((sub) => (
                        <Link
                          ...
                          href={sub.href}
                          onClick={() => setMobileMenuOpen(false)}
                          className="block px-4 py-2 text-sm text-gray-600 hover:bg-gray-50 hover:text-secondary-blue transition-colors"
                        >
                          {sub.name}
                        </Link>
                      ))}
              ...
              <Link
                href="/admin"
                onClick={() => setMobileMenuOpen(false)}
                className="block px-4 py-3 font-semibold text-sm text-gray-700 hover:bg-gray-50 hover:text-secondary-blue transition-colors border-t border-gray-200 mt-4 pt-4"
              >
                Admin
              </Link>
            </div>
